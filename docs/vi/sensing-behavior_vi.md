...
`FaceRecognizer._post_wellbeing` của LeLamp ghi thẳng row `enter` / `leave` qua `POST /api/wellbeing/log` — agent không tham gia, sensing handler của Lamp cũng không ghi nữa.
