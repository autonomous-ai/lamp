# Lumi Buddy — sign + notarize cho production
...
1. `https://account.apple.com/account/manage` → Sign-In and Security → **App-Specific Passwords** → Generate (đặt nhãn `lumi-buddy-notary` hoặc tương tự).
...
`lumi-notary` là alias profile — pass nó qua `NOTARY_PROFILE` khi `make`. Đặt tên gì cũng được, miễn nhất quán.
